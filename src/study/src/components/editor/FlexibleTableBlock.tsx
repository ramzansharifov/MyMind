...
export function FlexibleTableBlock({
  ...
  function requireRange(): TableCellRange | null {
    if (!activeRange) {
      showAppWarning("Ячейки не выбраны", "Сначала выбери одну или несколько ячеек таблицы.");
      return null;
    }
  ...
  function mergeSelectedCells() {
    ...
    if (!range || isSingleCellRange(range)) {
      showAppWarning("Нужно выделить диапазон", "Для объединения выбери минимум две ячейки.");
      return;
    }
    ...
    if (hasMergedCellInside) {
      showAppWarning("Нельзя объединить", "В выделении уже есть объединённые ячейки. Сначала раздели их.");
      return;
    }
    ...
    const nextBackups: TableCellMergeBackups = {
      ...cellMergeBackups,
      [masterKey]: backup,
    };
  ...
  function splitSelectedCells() {
    ...
    if (splitCount === 0) {
      showAppWarning("Нечего разделять", "В выделении нет объединённых ячеек.");
      return;
    }
    ...
    <div className="border border-black bg-white">
      {editable && (
        <div className="border-b border-black bg-neutral-100 px-3 py-2 text-xs text-neutral-600">
          Клик по ячейке — выбор одной ячейки. Shift + клик или маленький квадрат в углу — выделение диапазона.
        </div>
      )}
                  ...
                  {row.map((cell, columnIndex) => {
                    ...
                    return (
                      <CellTag
                        ...
                        className={[
                          "relative border border-black p-0 text-left align-top",
                          isSelected ? "table-cell-selected" : "",
                        ].join(" ")}
                      ...
                      >
                        {editable && (
                          <button
                            type="button"
                            aria-label="Выделить диапазон ячеек"
                            title="Потяни, чтобы выделить диапазон"
