...
function buildResults(data: AppData, query: string): SearchResult[] {
  ...
  data.contacts.filter((item) => !isHiddenFromRegularLists(item)).forEach((item) => add({ id: `contact-${item.id}`, module: 'contacts', title: item.name, detail: item.relationship }, `${item.name} ${item.notes} ${item.tags.join(' ')}`));
