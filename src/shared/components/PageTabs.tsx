...
export function PageTabs<T extends string>({
  ...
  return (
    <Tabs.Root value={activeTab} onValueChange={(value) => onChange(value as T)}>
      <Tabs.List
        className={cn(
        ...
        {tabs.map((tab) => (
          <Tabs.Trigger
            className={cn(
              ...
              'data-[state=active]:border-[color-mix(in_srgb,var(--accent)_44%,transparent)] data-[state=active]:bg-[color-mix(in_srgb,var(--accent)_16%,var(--surface-strong))] data-[state=active]:text-app-accent-strong',
              fullWidth && 'flex-1',
            )}
            key={tab.id}
            value={tab.id}
