import type { StudyCustomBlockTemplate, StudyMaterial, StudyNode } from "../types";
import { getStudyBlockText } from "../studyUtils";
...
export function getMaterialOutgoingLinks(
  ...
  const allText = material.blocks
    .map((block) => getStudyBlockText(block))
    .join("\n");
...
export function getMaterialBrokenLinks(
  ...
  return getMaterialOutgoingLinks(material, nodes).filter((link) => !link.found);
}
