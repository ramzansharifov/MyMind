...
interface StudyPageProps {
  ...
  onEditorActionsChange?: (actions: StudyEditorNavigationActions | null) => void;
}

type StudyMode = 'edit' | 'read';
type SaveMaterialOptions = { source?: 'manual' | 'auto' | 'flush' };

interface StudyEditorNavigationActions {
...
export function StudyPage({ data, onChange, onEditorDirtyChange, onEditorActionsChange }: StudyPageProps) {
  ...
  const lastReconciledStudySignatureRef = useRef<string | null>(null);
  const pendingStudyTreeSaveRef = useRef<Promise<void>>(Promise.resolve());
  ...
  useEffect(() => {
    return () => {
      ...
      if (hasUnsavedChangesRef.current && activeMaterialRef.current) {
        void saveMaterial({ source: 'flush' });
      }
  ...
  useEffect(() => {
    ...
    async function loadMaterial() {
      ...
      try {
        let material = await studyStorageClient.getMaterial(selectedMaterialId);
  ...
  useEffect(() => {
    if (lastReconciledStudySignatureRef.current === studyTreeSignature) return;
    ...
    async function reconcileMaterialIndex() {
      try {
        ...
        lastReconciledStudySignatureRef.current = studyTreeSignature;
  ...
  async function flushPendingStudySaves() {
    const materialSaved = await flushPendingMaterialSave();
    ...
    try {
      await pendingStudyTreeSaveRef.current;
      return true;
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'Не удалось сохранить структуру обучения.');
      return false;
  ...
  useEffect(() => {
    onEditorDirtyChange?.(hasUnsavedChanges);
  }, [hasUnsavedChanges, onEditorDirtyChange]);

  useEffect(() => {
    if (!onEditorActionsChange) return;
  ...
  }, [activeMaterial?.id, onEditorActionsChange]);
